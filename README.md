...
| **设备操作** | 投屏、Shell（聚焦设备 logcat）、重启、断开、离线重连/移除；拖放 APK 单设备安装 |
...
- 交互式 `adb shell` 终端（当前 Shell = 聚焦 logcat，非 REPL）  
- 崩溃遥测上报（设置页已标注「即将推出」）  
...
4. **查看 Logcat：** 侧栏/底栏打开 Logcat 控制台 → 切换到 **Logcat** 标签；点击设备 **Shell** 可只看该设备日志。  
5. **批量运维：** 导航至「分组命令」，选择目标分组后执行批量操作。  
6. **USB + WiFi 双连接：** 同一台手机若 USB 与无线同时在线，设备墙会自动按硬件序列号去重，优先展示 WiFi 条目。
